...
def loss(asr_encoder_state,
  ...
  with ops.name_scope(name, "loss", [asr_encoder_state, encoder_state]):
    if loss_type == "MSE":
      ...
      cost = tf.reduce_mean(losses) * 1024
    elif loss_type == "cosine_dist":
      losses = []
      for i in range(len(encoder_state)):
        norm_encoder_state = tf.nn.l2_normalize(encoder_state[i], dim=1)
        norm_asr_encoder_state = tf.nn.l2_normalize(asr_encoder_state[i], dim=1)
        losses.append(tf.losses.cosine_distance(norm_encoder_state, norm_asr_encoder_state, dim=1))
...
def context_vector_loss(asr_encoder_state,
  ...
  with ops.name_scope(name, "context_vector_loss", [asr_encoder_state, encoder_state]):
    cost = loss(asr_encoder_state, encoder_state, 'MSE')
    return cost

def get_context_vector(encoder_inputs,
                       asr_encoder_inputs,
                       cell,
                       num_encoder_symbols,
                       embedding_size,
  ...
  with variable_scope.variable_scope(scope or "embedding_rnn_seq2seq") as scope:
    ...
    asr_encoder_state = tf.concat(asr_encoder_state, 1)
    return encoder_state, asr_encoder_state
...
def xavier_init(size):
  in_dim = math_ops.cast(size[0], tf.float32)
...
def discriminator(X, reuse=False):
  ...
  with tf.variable_scope("discriminator") as scope:
    ...
    D_W3 = tf.Variable(xavier_init([h2_dim, 1]))
